...
def display_live_match_visualization(live_data):
    ...
    def highlight_risk(val):
        """Apply conditional styling to risk score column"""
        if isinstance(val, (int, float)):
            # For risk_score column
            if 0 <= val <= 1:
                # Create a color gradient from green to red
                r = int(255 * val)
                g = int(255 * (1 - val))
                b = 0
                return f'background-color: rgba({r}, {g}, {b}, 0.8); color: white; font-weight: bold; border-radius: 5px; padding: 0.3rem 0.6rem; text-align: center;'
        return ''
    ...
    display_df = display_df[['Rank', 'Player Name', 'Risk Score']]
    ...
    # Apply risk score styling
    styled_df = styled_df.applymap(
        lambda x: highlight_risk(float(x.strip('%'))/100) if isinstance(x, str) and '%' in x else '',
        subset=['Risk Score']
    )
...
def create_trend_visualization(live_data):
    ...
    # Update layout
    fig.update_layout(
        title_text="Player Risk Ranking Trends",
        title_font_size=18,
